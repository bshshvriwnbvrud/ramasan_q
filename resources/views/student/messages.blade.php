...
<div class="container py-4">
    <div class="d-flex align-items-center mb-4" data-aos="fade-down">
        ...
        <span class="badge bg-warning text-dark me-3 px-3 py-2 rounded-pill">محادثة مع الدعم الفني</span>
    </div>
    ...
    <div class="card glass-card border-0" data-aos="fade-up">
        <div class="card-header bg-transparent d-flex align-items-center" style="border-bottom: 1px solid var(--glass-border);">
            <img src="https://ui-avatars.com/api/?name=Support&background=random&color=fff&size=40" class="rounded-circle me-2" alt="Support">
            <div>
                <h6 class="mb-0 text-white">فريق الدعم</h6>
                <small class="text-white-50">نحن هنا لمساعدتك</small>
            </div>
        </div>
        ...
        <div class="card-body p-0" style="height: 450px; overflow-y: auto;" id="messageContainer">
            ...
            @empty
                <div class="empty-messages">
                    ...
                    <p class="text-white-50">ابدأ بكتابة استفسارك وسيتم الرد عليك قريباً</p>
                </div>
            @endforelse
        </div>
        ...
        <div class="card-footer bg-transparent" style="border-top: 1px solid var(--glass-border);">
            <form id="messageForm" class="position-relative">
                ...
                <div class="input-group">
                    <textarea name="message" id="messageInput" class="form-control message-input" placeholder="اكتب استفسارك هنا..." rows="1" maxlength="500" required></textarea>
                ...
                <div class="d-flex justify-content-between mt-2">
                    <small class="text-white-50">
                        <i class="bi bi-info-circle me-1"></i>سيتم الرد عليك في أقرب وقت ممكن
                    </small>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // التمرير إلى أسفل عند تحميل الصفحة
        ...
        // دالة مساعدة لإضافة رسالة إلى الواجهة (مع التحقق من التكرار)
        function addMessageToContainer(message, type) {
            // التحقق من أن الرسالة غير موجودة بالفعل
            if (document.querySelector(`[data-message-id="${message.id}"]`)) {
                return; // موجودة بالفعل، لا نضيفها مرة أخرى
            }
        ...
        // ** التحديث التلقائي (Polling) مع منع التداخل **
        function pollMessages() {
            if (isPolling) return; // منع التداخل
            isPolling = true;

            fetch('{{ route("messages.poll") }}?last_id=' + lastMessageId, {
            ...
            .then(data => {
                if (data.messages && data.messages.length) {
                    data.messages.forEach(msg => {
                        const type = msg.sender_id == userId ? 'outgoing' : 'incoming';
                        addMessageToContainer(msg, type);
                        // تحديث lastMessageId إلى آخر رسالة فقط بعد المعالجة
            ...
            .finally(() => {
                isPolling = false;
                // جدولة الطلب التالي بعد 5 ثوانٍ من انتهاء هذا الطلب
        ...
        // إيقاف polling عند مغادرة الصفحة (اختياري)
